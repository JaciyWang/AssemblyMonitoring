...
public class MediaCaptureUnity : MonoBehaviour
{
    ...
    bool isRunning = true;
    void Start() {
        DebugLog(TAG + " Start() thread ID is " + Thread.CurrentThread.ManagedThreadId);
    ...
    IEnumerator GetFrame()
    {
        while (isRunning)
        {
            if (!textureInitialized && captureStatus == CaptureStatus.Initialized)
            {
                ...
                //mediaTexture = new Texture2D(256, 256, TextureFormat.RGBA32, false);
    ...
    async void GetFrameTextureAsyncWrapper()
    {
        ...
        if(data != null && data.Length > 1)
        {
            ...
            //calibrationText.GetComponent<TextMeshPro>().text = TAG + ": Trigger sending or saving";
        }
        else
        {
        ...
        using (var inputReference = input.CreateReference())
        {
            ...
            DebugLog(TAG + ": Trigger sending or saving");
            saveCount++;
        }*/
    }
    ...
    private void Start()
    {
        debugText.transform.GetComponent<TextMeshPro>().text = "Calibration not started";
        calibrationText.transform.GetComponent<TextMeshPro>().text = "Calibration not started";
    }
